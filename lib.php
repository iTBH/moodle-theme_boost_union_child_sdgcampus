...
// Constants which are used throughout this theme.
define('THEME_SDG_BOOST_UNION_SETTING_INHERITANCE_INHERIT', 0);
define('THEME_SDG_BOOST_UNION_SETTING_INHERITANCE_DUPLICATE', 1);
 ...
 * @return string
 */
function theme_sdg_boost_union_get_main_scss_content($theme) {
    global $CFG;
    ...
    $scss .= file_get_contents($CFG->dirroot . '/theme/sdg_boost_union/scss/post.scss');
 ...
 * @return string
 */
function theme_sdg_boost_union_get_pre_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_sdg_boost_union', 'prescssinheritance');
    if ($inheritanceconfig == THEME_SDG_BOOST_UNION_SETTING_INHERITANCE_DUPLICATE) {
        $scss .= theme_boost_union_get_pre_scss(theme_config::load('boost_union'));
    }

    // And add Boost Union Child's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/sdg_boost_union/scss/pre.scss');
 ...
 * @return string
 */
function theme_sdg_boost_union_get_extra_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_sdg_boost_union', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_SDG_BOOST_UNION_SETTING_INHERITANCE_DUPLICATE) {
