...
class section_nav extends external_api {
    ...
    public static function execute(int $courseid, int $section) {
        ...
        $sectioninfo = $modinfo->get_section_info_by_id($section);

        $navigationlinks = navigation_helper::get_navigation_links($courseid, $section, 'section');
    ...
    private static function generate_navigation_link($courseid, $element, $isnext) {
        ...
        if ($element['type'] === 'section') {
            ...
            $link = new moodle_url("/course/section.php", ['id' => $element['id']]);
            ...
            if ($isnext) {
                ...
                $name = get_string('nextsection', 'theme_sdg_boost_union');
                ...
                $name = html_writer::tag('span', $arrow, ['class' => 'larrow mr-2']) .
                    get_string('prevsection', 'theme_sdg_boost_union');
                if (!$sectioninfo->visible) {
                    $name .= ' ' . get_string('hiddenwithbrackets');
                }
            }
            ...
            if ($isnext) {
                ...
                $name = get_string('nextactivity', 'theme_sdg_boost_union');
                if (!$cm->visible) {
                    $name .= ' ' . get_string('hiddenwithbrackets');
                }
                ...
                $name = html_writer::tag('span', $arrow, ['class' => 'larrow mr-2']) .
                    get_string('prevactivity', 'theme_sdg_boost_union');
                if (!$cm->visible) {
                    $name .= ' ' . get_string('hiddenwithbrackets');
                }
            }
